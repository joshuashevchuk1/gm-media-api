...
absl::StatusOr<VideoAssignmentChannelToClient>
VideoAssignmentResourceHandler::ParseUpdate(absl::string_view update) {
  DLOG(INFO) << kVideoAssignmentResourceName
  ...
  VideoAssignmentChannelToClient resource_update;
  // Response
...
absl::StatusOr<std::string> VideoAssignmentResourceHandler::Stringify(
    const VideoAssignmentChannelFromClient& client_request) {
  ...
  if (client_request.request.set_video_assignment_request.has_value()) {
    ...
    std::vector<VideoCanvas> canvases =
        client_request.request.set_video_assignment_request->layout_model
            .canvases;
