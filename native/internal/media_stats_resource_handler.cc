...
absl::StatusOr<MediaStatsChannelToClient>
MediaStatsResourceHandler::ParseUpdate(absl::string_view update) {
  DLOG(INFO) << kMediaStatsResourceName
  ...
  MediaStatsChannelToClient resource_update;
  // Response
  if (const Json* response_field = FindOrNull(json_resource_update, "response");
    ...
    resource_update.response = std::move(response);
  }
  ...
  if (const Json* resources_field =
    ...
    const Json* id_field = FindOrNull(resource_field, "id");
    if (id_field == nullptr) {
      return absl::InternalError(
          absl::StrCat("Invalid ", kMediaStatsResourceName,
                       " json format. Expected non-empty id field"));
    }
    ...
    resource_update.resources = std::move(resources);
  }
  return resource_update;
}

absl::StatusOr<std::string> MediaStatsResourceHandler::Stringify(
    const MediaStatsChannelFromClient& client_request) {
  ...
  // Request.requestId
  json_request["request"]["requestId"] = client_request.request.request_id;
  // Request.uploadMediaStats
  if (client_request.request.upload_media_stats.has_value()) {
    // Request.uploadMediaStats.sections
    for (const MediaStatsSection& section :
         client_request.request.upload_media_stats->sections) {
