...
TEST(MeetMediaApiAudioDeviceModuleTest, SuccessfullyInvokeInitPlayout) {
  auto adm = rtc::make_ref_counted<MeetMediaApiAudioDeviceModule>();
  EXPECT_EQ(adm->InitPlayout(), 0);
  EXPECT_TRUE(adm->Playing());
}

TEST(MeetMediaApiAudioDeviceModuleTest, SuccessfullyInvokeStopPlayout) {
...
TEST(MeetMediaApiAudioDeviceModuleTest,
...
TEST(MeetMediaApiAudioDeviceModuleTest, InitPlayoutTwiceLogsError) {
  auto adm = rtc::make_ref_counted<MeetMediaApiAudioDeviceModule>();
