...
///
/// Note that this is a reference client. It is not intended to be a complete
/// SDK with full support, customization, nor optimizations of WebRTC and real
/// time communication.
///
...
struct MediaApiClientConfiguration {
  /// For values greater than zero, the Media API client will establish that
  /// many video SRTP streams. After the session is initialized, no other
...
class MediaApiClientObserverInterface : public webrtc::RefCountInterface {
 public:
  ...
  /// `SESSION_DISCONNECTED`, or if `LeaveConference` is called while the client
  /// is joining the conference. All other disconnections are considered
  /// ungraceful (PeerConnection closed, Meet servers unreachable, etc).
  ///
...
class MediaApiClientInterface {
 public:
  ...
  /// will be called. If this method returns OK and joining fails, the
  /// observer's `OnDisconnected` method will be called. If the client
  /// successfully joins, `OnDisconnected` will be invoked when the client
  /// leaves the conference for whatever reason.
  ///
  ...
  /// the provided request ID. See `SendRequest` for more information.
  ///
